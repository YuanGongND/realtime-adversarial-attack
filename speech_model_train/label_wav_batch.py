...
def run_graph(wav_data, labels, input_layer_name, output_layer_name,
  ...
  with tf.Session() as sess:
    ...
    predictions_batch = sess.run(softmax_tensor, {input_layer_name: wav_data})
    
    #print(predictions)
    for i in range(0, predictions_batch.shape[0]):
...
def label_wav(wav, labels, graph, input_name, output_name, how_many_labels):
  ...
  fs, wav_data = wavfile.read(wav)
  wav_data = wav_data.reshape([16000, 1])/32768
  wav_data = np.zeros([16000, 2000])
  print(wav_data)
