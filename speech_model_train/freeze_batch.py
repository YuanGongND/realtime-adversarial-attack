...
def create_inference_graph(wanted_words, sample_rate, clip_duration_ms,
  ...
  batch_freeze_size = 50

  decoded_sample_data = tf.placeholder(tf.float32, [16000, batch_freeze_size], name='decode_wav')

  # the first audio
  spectrogram = contrib_audio.audio_spectrogram(
          tf.reshape(decoded_sample_data[:, 0], [16000, 1]),
  ...
  all_input = tf.reshape(fingerprint_input, [-1, fingerprint_size])
  
  for i in range(1, batch_freeze_size):
      spectrogram = contrib_audio.audio_spectrogram(
          tf.reshape(decoded_sample_data[:, i], [16000, 1]),
          window_size=model_settings['window_size_samples'],
          stride=model_settings['window_stride_samples'],
          magnitude_squared=True)
      ...
      fingerprint_size = model_settings['fingerprint_size']
      this_input = tf.reshape(fingerprint_input, [-1, fingerprint_size])
      
      all_input = tf.concat([all_input, this_input], axis=0)

  logits = models.create_model(
      all_input, model_settings, model_architecture, is_training=False,
